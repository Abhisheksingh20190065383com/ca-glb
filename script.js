// script.js - Final GLB Studio (polished & robust)
...
/* Diagnostics */
console.log('GLB Studio initializing - checking DOM elements', {
  viewer: !!viewer, dropArea: !!dropArea, fileInput: !!fileInput, chooseBtn: !!chooseBtn
});
...
function initScene(){
  ...
  const w = Math.max(400, rect.width || window.innerWidth * 0.6);
  const h = Math.max(300, rect.height || window.innerHeight * 0.55);
  camera = new THREE.PerspectiveCamera(60, w/h, 0.1, 2000);
  camera.position.set(3,2.2,4);

  renderer = new THREE.WebGLRenderer({ antialias:true, alpha:true, preserveDrawingBuffer:true });
  renderer.setPixelRatio(window.devicePixelRatio);
  renderer.setSize(Math.max(300, rect.width || w), Math.max(240, rect.height || h));
...
function onResize(){
  const rect = viewer.getBoundingClientRect();
  const w = Math.max(300, rect.width || window.innerWidth * 0.55);
  const h = Math.max(240, rect.height || window.innerHeight * 0.5);
...
function animate(){
  ...
  const delta = Math.min(0.04, (performance.now() - (window._lastFrame || performance.now())) / 1000);
...
function setupUI(){
  ...
  document.getElementById('screenshot')?.addEventListener('click', ()=> {
    ...
    }catch(e){ console.error(e); showToast('Screenshot failed'); }
  });
...
function handleFile(file){
  ...
  showToast && showToast('Loading ' + file.name + '...');
  ...
  } catch(e) { console.warn('DRACO init failed (optional):', e); }

  const onLoad = (gltf) => {
    try {
      clearScene();
      currentRoot = gltf.scene || gltf.scenes?.[0] || new THREE.Group();
      currentRoot.name = file.name || 'Model';
      ...
      fitCameraToObject && fitCameraToObject(currentRoot, 1.4);
      updateModelInfo && updateModelInfo(currentRoot, gltf);
      showToast && showToast('Loaded: ' + file.name);
      console.log('Model loaded OK', file.name, gltf);
    } catch(err) {
      console.error('onLoad handling error', err);
      showToast && showToast('Load handler error');
    }
  };
  ...
  const onError = (err) => {
    ...
    alert('Model load failed — check console for error. If file is .gltf with external resources, run a local server (see README).');
    showToast && showToast('Load failed');
  };
  ...
  if (ext === 'glb' || ext === 'bin') {
    ...
    reader.onload = (e) => {
      try {
        ...
        loader.parse(text, '', onLoad, (err) => {
          console.warn('parse failed, fallback to URL:', err);
          const url = URL.createObjectURL(file);
          loader.load(url, onLoad, onProgress, onError);
        });
      } catch(parseErr) {
        console.error('File read/parse error:', parseErr);
        onError(parseErr);
      }
    };
    reader.onerror = (e) => { console.error('FileReader error', e); onError(e); };
    reader.readAsText(file);
  } else {
    alert('Unsupported file type: ' + ext + '\\nUse .glb or .gltf (for .gltf with external files, run a local server).');
    showToast && showToast('Unsupported file type');
  }
}
...
/* HDRI loader (Poly Haven example) */
function loadHdri(){
